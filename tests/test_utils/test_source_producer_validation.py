...
from tests.test_utils.validation import validate_source_producer, InvalidArgumentException
...
@pytest.mark.unit
def test_validate_source_producer_direct():
    """Test validating a SourceProducer directly."""
    sp = SourceProducer(organization="testorg", product="testprod", module="testmodule")
    ...
    with pytest.raises(AssertionError, match="Expected organization 'wrongorg'"):
        validate_source_producer(
            sp,
            expected_organization="wrongorg",
            expected_product="testprod"
        )
...
def test_validate_source_producer_in_model():
    ...
    with pytest.raises(AssertionError, match="Handler key 'NonExistentPlugin' not found"):
        validate_source_producer(
            model,
            expected_organization="testorg",
            expected_product="testprod",
...
@pytest.mark.unit
def test_validate_source_producer_in_dict():
    ...
    with pytest.raises(AssertionError, match="SourceProducer not found in dictionary"):
        validate_source_producer(
            {},
            expected_organization="testorg",
            expected_product="testprod"
...
@pytest.mark.unit
def test_validate_source_producer_invalid_input():
    """Test validating a SourceProducer with invalid input types."""
    # Should fail validation with string input
    with pytest.raises(AssertionError, match="Input of type <class 'str'> cannot be validated"):
        validate_source_producer(
            "not a valid input",
            expected_organization="testorg",
            expected_product="testprod"
    ...
    with pytest.raises(AssertionError, match="Input of type <class 'NoneType'> cannot be validated"):
        validate_source_producer(
            None,
            expected_organization="testorg",
            expected_product="testprod"
